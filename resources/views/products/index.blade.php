<x-app-layout>
    ...
    <div class="p-6 flex flex-col gap-6 max-w-7xl mx-auto">
        ...
        <div class="p-4 bg-gray-100 rounded-md shadow-md">
            <form method="GET" action="{{ route('product.index') }}" class="flex flex-col gap-4 md:flex-row md:items-end">
                {{-- Filter Kategori --}}
            ...
            @foreach ($products as $product)
                <div class="bg-gray-100 border my-4 p-4 rounded-lg shadow-[0px 14px 34px 0px rgba(0,0,0,0.08)] dark:bg-zinc-900 dark:text-white/70 flex justify-between gap-6">
                    <div>
                        ...
                        <p class="text-xl mt-4 font-reguler">{{ Str::title($product->status->name) }}</p>
                        <p class="text-2xl mt-2 font-extrabold">Rp. {{ number_format($product->price, 0, ',', '.') }}</p>
                    </div>
